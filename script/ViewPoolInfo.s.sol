...
contract ViewPoolInfo is Script {
    ...
    function _getPoolHealthInfo() internal view {
        ...
        try stateView.getSlot0(poolId) returns (uint160 sqrtPriceX96, int24 tick, uint24 protocolFee, uint24 lpFee) {
    ...
    function _getFeeInfo() internal view {
        ...
        try stateView.getFeeGrowthGlobals(poolId) returns (uint256 feeGrowthGlobal0X128, uint256 feeGrowthGlobal1X128) {
    ...
    function _getTickInfo() internal view {
        ...
        for (uint256 i = 0; i < ticksToCheck.length; i++) {
    ...
    function _getPositionInfo() internal view {
        ...
        try stateView.getPositionLiquidity(poolId, positionId) returns (uint128 liquidity) {
